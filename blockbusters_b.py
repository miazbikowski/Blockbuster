...
class Store:
	...
	def rent_movie(self, customer_id, movie_id):
		...
		rental_date = datetime.datetime.now().strftime('%d/%m/%Y, %H:%M:%S') #??? rental_date and return_date dont use the same format... should check that --> the difference it that the return date is an update on an entry
